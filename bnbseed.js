...
contract BNBSeed {
	...
	constructor(address payable wallet) public {
        ...
        plans.push(Plan(10, 51840));
		plans.push(Plan(15, 80640));
		plans.push(Plan(30, 109440));
        plans.push(Plan(21, 288000));
        plans.push(Plan(7, 288000));
	}
	...
	function getyourbonusminmoney(address userAddress) public view returns (uint256) {
		...
		for (uint256 i = 0; i < users[msg.sender].deposits.length; i++) {
			if(user.deposits[i].plan < 3){
				if(user.deposits[i].start.add(plans[user.deposits[i].plan].time.mul(1 minutes)) > block.timestamp){
					if(user.deposits[i].plan == 0){
						yourbonusminmoney = yourbonusminmoney.add(users[msg.sender].deposits[i].amount*18/1000);

					}else if(user.deposits[i].plan == 1){
						yourbonusminmoney = yourbonusminmoney.add(users[msg.sender].deposits[i].amount*28/1000);

					}else if(user.deposits[i].plan == 2){
						yourbonusminmoney = yourbonusminmoney.add(users[msg.sender].deposits[i].amount*38/1000);
